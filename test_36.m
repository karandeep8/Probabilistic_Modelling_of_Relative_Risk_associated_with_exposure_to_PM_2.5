...
a = 1.22310629;
min_value_parameter = 0.81759043199;
...
% Find P(Y <= c0), P(Y <= c1), and P(Y <= c2) using the CDF
...
line([min(X), c0], [P_Y_le_c0, P_Y_le_c0], 'Color', 'black', 'LineStyle', '--', 'LineWidth', 2.5); % Horizontal line at P(Y <= c0)
...
line([min(X), c1], [P_Y_le_c1, P_Y_le_c1], 'Color', 'red', 'LineStyle', '--', 'LineWidth', 2.5); % Horizontal line at P(Y <= c1)
...
line([min(X), c2], [P_Y_le_c2, P_Y_le_c2], 'Color', 'blue', 'LineStyle', '--', 'LineWidth', 2.5); % Horizontal line at P(Y <= c2)
...
% Move the x-axis annotations further down by using a lower y-value.
text(c0, -0.15, sprintf('c0 = %.2f', c0), ...
    'FontSize', 14, 'FontWeight', 'bold', 'Color', 'black', ...
    'HorizontalAlignment', 'center', 'VerticalAlignment', 'top');
text(min(X) + 0.1*(max(X)-min(X)), P_Y_le_c0, sprintf('P(Y \\leq c0) = %.4f', P_Y_le_c0), ...
    'FontSize', 14, 'FontWeight', 'bold', 'Color', 'black', ...
...
text(c1, -0.15, sprintf('c1 = %.2f', c1), ...
    ...
    'HorizontalAlignment', 'center', 'VerticalAlignment', 'top');
text(min(X) + 0.1*(max(X)-min(X)), P_Y_le_c1, sprintf('P(Y \\leq c1) = %.4f', P_Y_le_c1), ...
    'FontSize', 14, 'FontWeight', 'bold', 'Color', 'red', ...
    'HorizontalAlignment', 'left', 'VerticalAlignment', 'bottom');
...
text(c2, -0.15, sprintf('c2 = %.2f', c2), ...
    ...
    'HorizontalAlignment', 'center', 'VerticalAlignment', 'top');
text(min(X) + 0.1*(max(X)-min(X)), P_Y_le_c2, sprintf('P(Y \\leq c2) = %.4f', P_Y_le_c2), ...
    'FontSize', 14, 'FontWeight', 'bold', 'Color', 'blue', ...
    'HorizontalAlignment', 'left', 'VerticalAlignment', 'bottom');

% Add labels and formatting
graph_title = sprintf('CDF of RR (%s, %d, \\beta = %.4f)', country, year, beta);
