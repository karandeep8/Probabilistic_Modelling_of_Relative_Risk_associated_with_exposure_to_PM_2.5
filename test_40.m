...
% Find P(Y <= c0), P(Y <= c1), and P(Y <= c2) using the CDF, with bounds
P_Y_le_c0 = max(0, min(1, interp1(X, rr_cdf_values, c0, 'linear', 'extrap')));
P_Y_le_c1 = max(0, min(1, interp1(X, rr_cdf_values, c1, 'linear', 'extrap')));
P_Y_le_c2 = max(0, min(1, interp1(X, rr_cdf_values, c2, 'linear', 'extrap')));

% Display the results
fprintf('P(Y <= c0) where c0 = %.2f: %.4f\n', c0, P_Y_le_c0);
fprintf('P(Y <= c1) where c1 = %.2f: %.4f\n', c1, P_Y_le_c1);
fprintf('P(Y <= c2) where c2 = %.2f: %.4f\n', c2, P_Y_le_c2);
